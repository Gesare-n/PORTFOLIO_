...
const SkillsCanvas = dynamic(() => import("@/components/skills-canvas"), {
  ssr: false,
  loading: () => (
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {filteredSkills.map((skill) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="bg-gray-800 rounded-lg p-4 text-center skill-icon"
              style={{ backgroundColor: `${skill.color}20` }}
            >
              <div
                className="w-12 h-12 mx-auto mb-2 flex items-center justify-center rounded-lg"
                style={{ backgroundColor: skill.color }}
              >
                <span className="text-white font-bold">{skill.icon}</span>
              </div>
